...
void selectionsort(int n, int *ptr) {
  int i, j, t;
  for (i = 0; i < n; i++) {
    for (j = i + 1; j < n; j++) {
      if (*(ptr + j) < *(ptr + i)) {
        ...
        // na funcao selectionsort somente *ptr é ponteiro, é realmente o enderço da memória
      }
    }
  }
  printf("SELECTION sort vector:");
  printArray(n, ptr);
  printf("\n");
}
...
// fault caso não use malloc ou calloc)
{
  ...
  clock_t begin = clock();
  selectionsort(n, vet); // essa função é o bubble sort?
  ...
  printf("\n SELECTION end time1: %f", time_spent2);
